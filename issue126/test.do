...
cd "C:\Users\B18945\Documents\xtevent\issues\126"

cap log close

cap log using test_without_marksample.log, replace text
*cap log using test_with_marksample.log, replace text
...
* insert missing values in a dependent variable
*policyvar follows staggered adoption, so missing values will not make impute to fail 
sort rid
replace eta=. in 1/4000
drop rid
*/
...
cap xtevent y in 1/600 if i<30 , panelvar(i) timevar(t) policyvar(z) window(5) plot
...
cap xtevent y in 1/600 if i<30 , panelvar(i) timevar(t) policyvar(z) window(5) proxy(x) plot
