		...
		if "`reghdfe'"=="" {
			...
			*if xtivreg, warn the user about robust estandar errors equivalent to vce(cluster panelvar)
			if "`cmd'"=="xtivreg" & (("`vce_orig'"=="robust" | "`vce_orig'"=="r") | "`rob_orig'"!=""){
				*di as text _n "You specified to compute robust standard errors and the underlying estimation command is {bf:xtivreg}. Standard errors will be equivalent as specifying {bf:vce(cluster panelvar)} option. See {help xtivreg}"
